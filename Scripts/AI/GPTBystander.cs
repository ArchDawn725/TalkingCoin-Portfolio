...
public class GPTBystander : GPT
{
    ...
    {
        string interuption = "";
        if (LLM.voiceController.IsSpeaking) { interuption = "**You were interrupted while you were speaking**"; message = interuption + message; }
    ...
    private async Task GetInterpretation(string message)
    {
        ...
        try
        {
            var response = await openai.CreateChatCompletion(new CreateChatCompletionRequest() { Model = "gpt-4o-mini", Messages = localMessages });
    ...
    private void UpdateAttitudeBasedOnResponse(OpenAI.CreateChatCompletionResponse response)
    {
        if (response.Choices != null && response.Choices.Count > 0)
        {
            ...
            LLM.animationController.Reaction = numbers[0];
        }
        else
        {
    ...
    {
        return "You are in medieval Britain, **speak in period-appropriate language**. " + setting;//  You see the merchant (the player) getting escorted by the guards to face trail for a crime, try not to interrupt or intervene.";
    }
    private string GetStaticPrompt()
    {
        return
            ...
            "Guidelines: Keep answers short (max 1 sentence). Don't describe your character's actions or thoughts. Player: "
            ;
    }
    private string GetInterpretationPrompt(string message)
    {
        return
             "Manager Instruction:" + "\n" +
             "Respond to this message only with a **comma-separated list of numbers**: -1, 0, or 1, based on the criteria below. Do not include any labels, text, or extra characters in your response, only the numbers." + "\n\n" +
             ...
             "2: Indicate if the player is being repetitive, asking for te same thing over and over without any changes. -1 for this is happening, 0 for not." + "\n" +
             "Message: " + message;
    }
}
